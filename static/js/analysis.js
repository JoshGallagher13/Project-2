//Function to create hexcolor codes based on string
...
function buildCharts(selected_genre) {
    d3.json('/api/game_player_summary').then(data => {
        ...
        scoreFilter = filteredData.filter(d => d['critic_score'] != null);
        ...
        var playerTot = player_count.reduce((a, b) => a + b, 0),
        hours = filteredData.map(d => d['hours_sum']);
        var hoursTot = hours.reduce((a, b) => a + b, 0),
        avgHours = (hoursTot/games.length),
        rank = filteredData.map(d => d['rank']);
        var avgGameHours = []
        ...
        var trace1 = {
            ...
            var layout = {
                title: 'Hours per Player vs. Sales Rank',
                showlegend: false,
                xaxis: {
                    title: 'Sales Rank',
            ...
            //if (games[gameMax(avgGameHours)] == games[gameMax(rank)]) {
...
d3.json('/api/get_genres').then(data => {
    ...
    //genreFilter = genres.filter(d => d['genre'] != null)
    records.length--;
    genres.length--;
